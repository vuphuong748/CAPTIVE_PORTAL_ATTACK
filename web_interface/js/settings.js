	...
	for (var key in settingsJson) {
		...
		if (settingsJson.hasOwnProperty(key)) {
			...
			if (typeof settingsJson[key] == "boolean") {
				...
				html += "<input class='input-color' style='background: #023500;color: #00FF00; font-weight: bold; border:none; width: 10em;height: 1.3rem;' type='number' name='" + key + "' value=" + settingsJson[key] + " onchange='save(\"" + key + "\",parseInt(this.value))'>";
			} else if (typeof settingsJson[key] == "string" && key != _language_key) {
				var _value = settingsJson[key];
				console.log(_value)
				html += "<input class='input-color' type='text' name='" + key + "' value='" + _value + "' " + (key == "version" ? "readonly" : "") + " onchange='save(\"" + key + "\",this.value)'>";
			} else if (typeof settingsJson[key] == "string" && key == _language_key) {
