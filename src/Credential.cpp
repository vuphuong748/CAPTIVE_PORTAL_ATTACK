...
Credential::Credential() {
  ...
  listGmail = new SimpleList<Gmail>;
...
void Credential::init() {
  ...
  // listFacebook
  if (isExist(root, str(CLI_GMAIL_CREDENTIAL)) &&
    ...
    if (isExist(root, str(CLI_GMAIL_CREDENTIAL))) {
      JsonArray &values = root[str(CLI_GMAIL_CREDENTIAL)];
      for (unsigned int i = 0; i < values.size(); i++) {
        add(str(CLI_GMAIL_CREDENTIAL), values[i][0], values[i][1]);
      }
    }
...
int Credential::count(String key) {
  ...
  } else if (key == str(CLI_GMAIL_CREDENTIAL)) {
    return listGmail->size();
...
String Credential::getGmail(int id) { return listGmail->get(id).gmail; }
String Credential::getName(int id) { return listGmail->get(id).name; }
...
void Credential::deleteAll() {
  String data =
      "{\"facebook\":[],\"gmail\":[],\"zalo\":[],\"twitter\":[],\"wifi\":[]}";
  if (!writeFile(FILE_PATH, data)) {
    prnt(F_ERROR_SAVING);
    prntln(FILE_PATH);
  }
